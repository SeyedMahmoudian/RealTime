...
int main(int argc, char *argv[]) {
	...
	while(terminate!=TRUE){
		int option=0;
		printf("1)To send a message to server.\n2)Quit\n");
		fflush(stdout);
		...
		case 1:
			...
			break;
		case 2:
			client.option=2;//set the option to 2
...
void validate(char* input,clientMsg* msg){
	...
	if(strlen(input)>2){
		...
		if(isdigit(msg->option)){ //if it is a digit if yes it means double digit was entered
		...
		}else if(isalpha(msg->option)){//a string was entered set the flag to 2
			msg->optionFlag='2';
			flag=FALSE;
		}
	}
		...
		msg->option=input[0];
		if(isdigit(msg->option)){//single digit was entered
			msg->optionFlag='1';
			msg->digitOption=atoi(input);
			flag=FALSE;
		}else if(isalpha(msg->option)){//single char was entered
			msg->optionFlag='2';
			flag=FALSE;
		}
	}
